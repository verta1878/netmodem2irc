...
uses
  Windows, SysUtils, Messages, Classes, Graphics, Controls, Forms, Dialogs, FPCCompat, LCLType, LMessages,
  SetupForm, StdCtrls, Struct, DebugStruct, Int64Em, CmnFunc, CmnFunc2,
  SetupTypes, ScriptRunner, BidiUtils, RestartManager, LResources;
...
{$ENDIF}
  if (GetWindowLong(GetAppHandle, GWL_EXSTYLE) and WS_EX_TOOLWINDOW = 0) <> AVisible then begin
    SetWindowPos(GetAppHandle, 0, 0, 0, 0, 0, SWP_NOSIZE or
      SWP_NOMOVE or SWP_NOZORDER or SWP_NOACTIVATE or SWP_HIDEWINDOW);
    ExStyle := GetWindowLong(GetAppHandle, GWL_EXSTYLE);
    if AVisible then
      ExStyle := ExStyle and not WS_EX_TOOLWINDOW
    else
      ExStyle := ExStyle or WS_EX_TOOLWINDOW;
    SetWindowLong(GetAppHandle, GWL_EXSTYLE, ExStyle);
    if AVisible then
      { Show and activate when becoming visible }
      ShowWindow(GetAppHandle, SW_SHOW)
    else
      SetWindowPos(GetAppHandle, 0, 0, 0, 0, 0, SWP_NOSIZE or
...
begin
  ...
  try
    ...
    try
      ...
      try
        ...
        { GetAppHandle is now known to be the main window. Set the shutdown block reason. }
        ShutdownBlockReasonCreate(GetAppHandle, SetupMessages[msgWizardInstalling]);
  ...
  ShutdownBlockReasonCreate(GetAppHandle,
...
begin
  ...
  ShutdownBlockReasonDestroy(GetAppHandle);
...
begin
  ...
  { Application.HookMainWindow(MainWindowHook); } { not in LCL }
...
begin
  { Application.UnhookMainWindow(MainWindowHook); } { not in LCL }
  inherited;
end;
...
begin
  ...
  if InstallMode = imNormal then begin
    ...
    SetActiveWindow(GetAppHandle);  { ensure taskbar button is selected }
    WizardForm.Show;
  end
  else
  ...
  case Message.Msg of
    WM_WINDOWPOSCHANGED: begin
        { When the application window is minimized or restored, also hide or
          ...
          window is GetAppHandle. }
        IsIcon := IsIconic(GetAppHandle);
