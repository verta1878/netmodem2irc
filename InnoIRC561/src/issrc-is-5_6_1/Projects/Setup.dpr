...
uses
  ...
  InnoComponentReg in 'InnoComponentReg.pas';
...
{$IFDEF IS_D12}
  { Delphi 2009 initially sets WS_EX_TOOLWINDOW on the application window.
    That will prevent our ShowWindow(GetAppHandle, SW_SHOW) calls from
    actually displaying the taskbar button as intended, so clear it. }
  SetWindowLong(GetAppHandle, GWL_EXSTYLE,
    GetWindowLong(GetAppHandle, GWL_EXSTYLE) and not WS_EX_TOOLWINDOW);
{$ENDIF}

  try
    SetErrorMode(SEM_FAILCRITICALERRORS);
    DisableWindowGhosting;
    { Application.HookMainWindow - not in LCL }
    SelectMode;
  except
