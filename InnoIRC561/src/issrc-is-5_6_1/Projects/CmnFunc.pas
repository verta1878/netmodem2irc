...
uses
  Windows, Messages, SysUtils, Forms, Graphics, Controls, StdCtrls, Classes, FPCCompat;
...
{ This moves the application window (GetAppHandle) to the same monitor
...
begin
  ...
  if Assigned(MonitorFromWindow) and Assigned(GetMonitorInfo) then begin
    ...
    AppMonitor := MonitorFromWindow(GetAppHandle, MONITOR_DEFAULTTONEAREST);
    if MBMonitor <> AppMonitor then begin
      Info.cbSize := SizeOf(Info);
      if GetMonitorInfo(MBMonitor, Info) then begin
        GetWindowRect(GetAppHandle, OldRect);
        SetWindowPos(GetAppHandle, 0,
...
begin
  ...
  if IsIconic(GetAppHandle) or
     (GetWindowLong(GetAppHandle, GWL_STYLE) and WS_VISIBLE = 0) or
     (GetWindowLong(GetAppHandle, GWL_EXSTYLE) and WS_EX_TOOLWINDOW <> 0) then begin
    ActiveWindow := GetActiveWindow;
    WindowList := DisableTaskWindows(0);
    try
      { Note: DisableTaskWindows doesn't disable invisible windows.
        MB_TASKMODAL will ensure that GetAppHandle gets disabled too. }
...
{$ELSE}
    ...
    try
      ...
      WindowList := DisableTaskWindows(0);
      try
        Result := MessageBox(GetAppHandle, Text, Caption, Flags);
      finally
        EnableTaskWindows(WindowList);
        SetActiveWindow(ActiveWindow);
      end;
...
begin
  if GetActiveWindow = GetAppHandle then
