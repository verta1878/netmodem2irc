...
procedure TWinControlParentBackground_R(Self: TWinControl; var T: Boolean); begin {$IF DEFINED(IS_D7) AND NOT DEFINED(FPC)} T := TWinControlAccess(Self).ParentBackground {$ELSE} T := False {$IFEND}; end;
procedure TWinControlParentBackground_W(Self: TWinControl; const T: Boolean); begin {$IF DEFINED(IS_D7) AND NOT DEFINED(FPC)} TWinControlAccess(Self).ParentBackground := T; {$IFEND} end;
