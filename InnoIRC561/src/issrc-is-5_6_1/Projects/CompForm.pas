...
uses
  ...
  DebugStruct, CompInt, UxThemeISX, FPCCompat, LCLType;
...
uses
  ...
  CompOptions, CompStartup, CompWizard, CompSignTools, CompTypes, LCLProc, LCLStrConsts;
...
begin
  ...
  FBuildImageList := ImageList_LoadImageA(GetModuleHandle(nil), 'BUILDIMAGES', 17, 0, clSilver, IMAGE_BITMAP, 0);
...
begin
  if IsWindowEnabled(GetAppHandle) then
...
begin
  Dir := GetWinDir;
  ShellExecute(GetAppHandle, 'open', PChar(AddBackslash(Dir) + 'explorer.exe'),
...
begin
  ShellExecute(GetAppHandle, 'open',
...
begin
  ShellExecute(GetAppHandle, 'open', 'http://www.innosetup.com/', nil,
    nil, SW_SHOW);
end;

procedure TCompileForm.HPSWebsiteClick(Sender: TObject);
begin
  ShellExecute(GetAppHandle, 'open', 'http://www.remobjects.com/ps', nil,
    nil, SW_SHOW);
end;
...
begin
  ShellExecute(GetAppHandle, 'open', 'http://www.jrsoftware.org/isdonate.php', nil,
    nil, SW_SHOW);
end;
...
begin
  ...
  try
    ...
    ShellExecuteResult := ShellExecuteExA(@Info);
...
begin
  ...
  if Changed then begin
    if IsWindowEnabled(GetAppHandle) then begin
