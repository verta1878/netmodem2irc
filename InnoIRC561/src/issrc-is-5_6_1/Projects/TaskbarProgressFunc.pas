...
uses
  Windows, {$IFDEF VER90} OLE2 {$ELSE} ActiveX {$ENDIF}, Forms, dwTaskbarList, FPCCompat;
...
begin
  if InitializeTaskbarList then
    TaskbarListInterface.SetProgressState(GetAppHandle, StateFlags[State]);
end;
...
begin
  if InitializeTaskbarList then begin
    ...
    Total64.Lo := Total;
    Total64.Hi := 0;
    TaskbarListInterface.SetProgressValue(GetAppHandle, Completed64, Total64);
  end;
end;
