...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, CommCtrl, LCLType, LMessages, FPCCompat;
...
begin
  ...
  try
    Result := SUCCEEDED(SHPathPrepareForWriteFunc(GetAppHandle, nil,
...
begin
  ...
  TreeView_SetImageList(Handle, LongWord(ImageList), TVSIL_NORMAL);

  { Add the root items }
  SaveCursor := Windows.SetCursor(Windows.LoadCursor(0, IDC_WAIT));
  try
