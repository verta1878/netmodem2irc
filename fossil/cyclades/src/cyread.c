         ...
         * The ReadDpc checks this to decide when to timeout. */
        {
            ...
            if (totalTimeoutMs > 0) {
                ...
                Irp->Tail.Overlay.DriverContext[1] =
                    (PVOID)(ULONG_PTR)((ULONG_PTR)tickCount.QuadPart +
                        (totalTimeoutMs / 16) + 1);
            } else {
...
VOID CyReadDpcComplete(PCY_PDO_EXT pdoExt)
{
    ...
    for (;;) {
...

        {
             ...
             * deadline is stored as ULONG_PTR in DriverContext.
             * Compare as ULONG_PTR (not ULONG) for x64 safety. */
            deadline = (ULONG_PTR)irp->Tail.Overlay.DriverContext[1];
            if (deadline != 0 &&
                (ULONG_PTR)tickCount.QuadPart >= deadline) {
                timedOut = TRUE;
            }
