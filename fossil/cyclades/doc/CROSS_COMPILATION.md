...
## OpenWatcom x64 Status

As of 2026, OpenWatcom IRC (development) builds have experimental
ELF64 output support via `-bt=linux64`. This targets Linux x86-64,
not Windows NT x64 kernel drivers.

For Windows x64 kernel drivers, the WDK remains the recommended
build environment. The WDK provides native amd64 compiler, x64
kernel headers/libraries, and proper PE32+ output.

OpenWatcom's `-bt=nt` targets i386 only. A future `-bt=nt64` flag
may be added when the OW x86-64 code generator matures.
