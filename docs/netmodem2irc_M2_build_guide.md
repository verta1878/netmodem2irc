...
This is the M2 step: turn engine + GUI source into a running NetModemServer.exe.

## Project files (prepared)
- server/NetModemServer.lpr — updated to `uses` the engine units + driver interface
- server/NetModemServer.lpi — NEW: sets the unit search paths so Lazarus finds:
    ../engine        (the netmodem2irc engine units)
    ../common        (NetModemVxD — driver interface)
...
1. Open server/NetModemServer.lpi in Lazarus.
...
3. Build (Default mode) → NetModemServer.exe. Run it → the GUI window appears.
...
NetModemServer.exe builds in Lazarus and launches showing the server window.
